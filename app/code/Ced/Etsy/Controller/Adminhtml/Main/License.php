...
namespace Ced\Etsy\Controller\Adminhtml\Main;
...
class License extends \Magento\Backend\App\Action
{
        ...
        if($postData){
            ...
            if ($response && isset($response['hash']) && isset($response['level'])) {
                ...
                try {
                    ...
                    {
                        $path = \Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_hash';
                        $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                        $path = \Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.strtolower($postData['module_name']).'_level';
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = (/*$this->_feedHelper->getStoreConfig(\Ced\Ebay\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? */'https://'/* : 'http://'*/)
