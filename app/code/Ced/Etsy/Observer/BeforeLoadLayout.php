...
class BeforeLoadLayout implements ObserverInterface
{
    ...
    public function __construct(
        ...
        $this->_feedHelper = $this->_objectManager->get('Ced\Etsy\Helper\Feed');
    }
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
       ...
       try {
            ...
            $helper = $this->_objectManager->create('Ced\Etsy\Helper\Feed');
            ...
            foreach ($modules as $moduleName=>$releaseVersion)
            {
                ...
                $h = $this->_objectManager->create('Ced\Etsy\Helper\Data')->getStoreConfig(\Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.$m.'_hash');
                for($i=1;$i<=(int)$this->_objectManager->create('Ced\Etsy\Helper\Data')->getStoreConfig(\Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.$m.'_level');$i++)
                    {  
                        $h = base64_decode($h);
                    }

                $h = json_decode($h,true);
                if(is_array($h) && isset($h['domain']) && isset($h['module_name']) && isset($h['license']) && strtolower($h['module_name']) == $m && $h['license'] == $this->_objectManager->create('Ced\Etsy\Helper\Data')->getStoreConfig(\Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.$m))
                 {}else{
            ...
            foreach ($result as $moduleName => $response) {
                if ($response && isset($response['hash']) && isset($response['level'])) {
                    ...
                    try {
                        ...
                        {
                            $path = \Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_hash';
                              $this->_feedHelper->setDefaultStoreConfig($path, $response['hash'], 0);
                            $path = \Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName).'_level';
                             $this->_feedHelper->setDefaultStoreConfig($path, $response['level'], 0);
                            $path = \Ced\Etsy\Block\Extensions::HASH_PATH_PREFIX.strtolower($moduleName);
    ...
    private function getLicenseActivateUrl()
    {
        if (is_null($this->_licenseActivateUrl)) {
            $this->_licenseActivateUrl = ($this->_feedHelper->getStoreConfig(\Ced\Etsy\Block\Extensions::LICENSE_USE_HTTPS_PATH) ? 'https://' : 'http://')
