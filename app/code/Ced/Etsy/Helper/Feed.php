...
class Feed extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(\Magento\Framework\App\Helper\Context $context ,
        ...
        $this->_allowedFeedType =  explode(',', $backendConfig->getValue(\Ced\Etsy\Model\Feed::XML_FEED_TYPES));
    ...
    public function isAllowedFeedType(\SimpleXMLElement $item) 
    {
        ...
        if(is_array($this->_allowedFeedType) && count($this->_allowedFeedType) >0) {
            ...
            case \Ced\Etsy\Model\Config\Type::TYPE_NEW_RELEASE :
            case \Ced\Etsy\Model\Config\Type::TYPE_INSTALLED_UPDATE :
                if (in_array(\Ced\Etsy\Model\Config\Type::TYPE_INSTALLED_UPDATE, $this->_allowedFeedType) && strlen(trim($item->module)) > 0 && isset($cedModules[trim($item->module)]) && version_compare($cedModules[trim($item->module)], trim($item->release_version), '<')===true) {
                    $isAllowed = true;
                    break;
                }
            case \Ced\Etsy\Model\Config\Type::TYPE_UPDATE_RELEASE :
                if(in_array(\Ced\Etsy\Model\Config\Type::TYPE_UPDATE_RELEASE, $this->_allowedFeedType) && strlen(trim($item->module)) > 0) {
                    $isAllowed = true;
                    break;
                }
                if(in_array(\Ced\Etsy\Model\Config\Type::TYPE_NEW_RELEASE, $this->_allowedFeedType)) {
                    $isAllowed = true;
                }
                break;
            case \Ced\Etsy\Model\Config\Type::TYPE_PROMO :
                if(in_array(\Ced\Etsy\Model\Config\Type::TYPE_PROMO, $this->_allowedFeedType)) {
                    $isAllowed = true;
                }
                break;
            case \Ced\Etsy\Model\Config\Type::TYPE_INFO :
                if(in_array(\Ced\Etsy\Model\Config\Type::TYPE_INFO, $this->_allowedFeedType)) {
                    $isAllowed = true;
                }
                break;
            }
    ...
    public function getEnvironmentInformation() 
    {
        ...
        $info['feed_types'] = $this->_backendConfig->getValue(\Ced\Etsy\Model\Feed::XML_FEED_TYPES);
