...
class Feed extends \Magento\AdminNotification\Model\Feed
{
    ...
    const XML_FEED_URL_PATH     = 'system/etsy/feed_url';
    const XML_FREQUENCY_PATH    = 'system/etsy/frequency';
    const XML_LAST_UPDATE_PATH  = 'system/etsy/last_update';
    ...
    public function checkUpdate()
    {
        ...
        $feedXml = $this->getFeedData($this->_objectManager->get('Ced\Etsy\Helper\Feed')->getEnvironmentInformation());
       
        $allowedFeedType = explode(',', $this->_backendConfig->getValue(self::XML_FEED_TYPES));
        
        if ($feedXml && $feedXml->channel && $feedXml->channel->item) {
            $installedModules=$this->_objectManager->get('Ced\Etsy\Helper\Feed')->getCedCommerceExtensions();
            foreach ($feedXml->channel->item as $item) {
               
                if(!isset($installedModules[(string)$item->module])) {
                    continue;
                }
                if($this->_objectManager->get('Ced\Etsy\Helper\Feed')->isAllowedFeedType($item)) {
